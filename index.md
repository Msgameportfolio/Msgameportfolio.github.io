...
- 시작화면, 게임화면, 설정화면, shop창 총 3개의 화면이 있다.
- 시작화면에는 시작하기와 shop창과 설정창이라는 버튼 3개가있다
- 시작하기 클릭시 게임화면으로 이동한다.
- 설정창 클릭시 설정화면으로 이동한다.
- 설정화면에서는 오디오, 해상도, 인터페이스등 조절할수있다. 
- shop창에서는 게임에 필요한 아이템을 제공함
- 게임화면에서 상단에는 각 문명의 과학,문화,신앙,금 산출량이 보여지고 중단에는 각 타일 국가가 문명을 개설하는 모습 하단에는 지도맵과 턴을 넘기는 ui삽입
- 과학승리는 기술&정책에 의해 영향을 받으며 최종 우주식민지  4단계를 건설시 승리하게되는 방식으로 진행된다. (기술&정책은 미정)
